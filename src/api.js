...
export const gameDetailsURL = (name) =>
  `https://api.rawg.io/api/games/${name}?key=${process.env.REACT_APP_API_KEY}`;
export const screenshotsURL = (name) =>
  `https://api.rawg.io/api/games/${name}/screenshots?key=${process.env.REACT_APP_API_KEY}`;
export const searchGamesURL = (name) =>
  `https://api.rawg.io/api/games?key=${process.env.REACT_APP_API_KEY}&page=2&page_size=9&search=${name}`;
